...
| User-provided local `archive.zip` skin-lesion collection | Offline-only seven-class research comparison; no raw data, cache, candidate checkpoint, or calibration artifact is in Git or Flask | Seven dermoscopic-style labels mapped to the current HAM10000 adapter | Archive folder labels only; 24,651 records after exact-hash grouping and collision exclusion | The local README cites component datasets but provides no machine-learning deployment licence record; source archive checksum is recorded in the run audit | No patient/case IDs; 641 files in original cross-split exact-duplicate groups; near-duplicate risk remains; no external, prospective, or clinical validation. The ResNet-18 candidate was not promoted. See [the 2026-09-22 experiment record](skin-lesion-zip-experiment-20260922.md). |
| SCIN (Skin Condition Image Network) | Enabled only for governed **offline research**; a prior two-class experiment remains **rejected** and no raw data, checkpoint, or calibration artifact is in Git or the app | Clinical-photo dermatology research, not dermoscopy transfer claims | Dermatologist weighted condition labels and available tone/demographic metadata; see the [official dataset documentation](https://github.com/google-research-datasets/scin) | SCIN Data Use License; reviewed before manifest-bound acquisition; retain attribution and never re-identify/re-link. The reproducible layout is declared in [`dataset_registry.json`](../dataset_registry.json). | Volunteer/self-submitted images, self-reported data, multi-condition/gradable limitations; no direct equivalence to Indian clinical population |
