...
SCIN_V1 = {
    ...
    "registry_manifest": "dataset_registry.json",
    ...
    "training_scope": "Offline research only after source-license review, manifest-bound acquisition, integrity audit, and recorded experiment review.",
...
EXPERIMENTS = {
    ...
    "scin_clinical_resnet18_20260909": {
        "experiment_id": "scin-clinical-resnet18-experiment-20260908T184548Z",
        "dataset_key": "scin_v1",
        "dataset_selection": "212 gradable clinical photos; one image_1 per SCIN case; strict single weighted dermatologist label >= 0.7; Eczema versus Urticaria only. Manifest excludes source demographics and free text.",
        "integrity_audit": "212/212 acquired and image-verified; 0 download errors; 0 exact duplicate groups across splits; 0 near-duplicate pairs at dHash Hamming distance <= 4.",
        "split": "148 train / 32 independent validation / 32 held-out test; case-grouped, not claimed patient-grouped.",
        "architecture": "ImageNet-initialised ResNet-18 with a frozen backbone and trained two-class head.",
        "calibration": "Temperature scaling fitted only on the 32-image validation split (T=0.925). Validation log loss improved 0.644813 to 0.644549 while validation ECE worsened 0.096573 to 0.103335.",
        "held_out_evaluation": {
            "balanced_accuracy": 0.41498,
            "macro_f1": 0.40567,
            "auroc_ovr_macro": 0.437247,
            "auprc_macro": 0.491965,
            "calibrated_log_loss": 0.850293,
            "calibrated_ece": 0.266993,
            "sample_count": 32,
        },
        "decision": "REJECTED_FOR_APPLICATION_INFERENCE",
        "reason": "A reproducibility re-run with stricter source gradability and image-integrity auditing performed worse than the earlier feasibility run. It remains a small, two-class study with no normal class, patient IDs, OOD detector, segmentation, external validation, or clinical validation.",
        "artifact_storage": "External research directory only; images, checkpoint, predictions, calibration artifact, and integrity audit are intentionally excluded from Git and from the Flask inference API.",
    },
