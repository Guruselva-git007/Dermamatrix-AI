...
class AssessmentStateTests(unittest.TestCase):
        ...
        for area in ("Skin", "Hair", "Nails"):
            for quality, score in (("GOOD", 12), ("LOW_QUALITY", 12), ("GOOD", 75)):
                with self.subTest(area=area, quality=quality, score=score):
                    ...
                    self.assertGreaterEqual(len(consumer["cause_sections"]), 3)
                    self.assertGreaterEqual(len(consumer["care_sections"]), 3)
                    self.assertGreaterEqual(len(consumer["routine_sections"]), 4)
                    self.assertGreaterEqual(len(consumer["treatment_sections"]), 3)
    ...
    def test_clear_hair_and_nail_images_return_a_real_non_diagnostic_result(self):
        ...
        for area, image_context, model_id in (
            ...
            self.assertTrue(consumer["cause_sections"])
