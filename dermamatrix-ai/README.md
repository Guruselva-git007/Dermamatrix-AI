...
`backend/risk_service.py` provides shared `LOW`, `MODERATE`, `HIGH`, `URGENT`, and `UNCERTAIN` semantics. `backend/pirs_service.py` is a transparent, configurable prototype aggregation; it is explicitly **not clinically validated**. Three local research adapters are currently installed: clinical Skin EfficientNet for declared ordinary photos, HAM10000 ResNet-34 for attested dermoscopy, and Nail ConvNeXt Tiny for declared nail close-ups. Their raw rankings are not diagnostic probabilities. Hair condition classification and Sweat image classification are unavailable; Sweat uses questionnaire rules. The separate earlier nail-model feasibility run failed its predeclared evaluation thresholds and was not promoted.
