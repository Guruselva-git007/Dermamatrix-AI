...
function normaliseAssessmentPresentation(data) {
  ...
  return {
    ...
    confidence: {
      value: isPresentationCase ? 'Not applicable' : likelihoodAvailable ? `${Math.round(likelihoodValue * 100)}%` : 'Not available',
      note: isPresentationCase ? 'This is an exact-file teaching match, not a model probability.' : likelihoodAvailable ? 'Calibrated research-model likelihood.' : 'A calibrated condition likelihood is not available for this assessment.',
    },
    severity: {
    ...
    assessmentRisk: {
      ...
      value: assessmentRisk.available && Number.isFinite(assessmentRisk.score) ? `${assessmentRisk.score}/100` : isPresentationCase ? 'Not applicable' : 'Not assessed',
      level: assessmentRisk.available ? (assessmentRisk.level ? readableStatus(assessmentRisk.level) : 'Not assessed') : isPresentationCase ? 'Teaching case' : (assessmentRisk.level ? readableStatus(assessmentRisk.level) : 'Not assessed'),
      urgency: isPresentationCase ? 'Teaching discussion' : (assessmentRisk.urgency_label || readableStatus(assessmentRisk.urgency, 'Routine monitoring')),
      factors: assessmentRisk.factor_labels || (assessmentRisk.factors || []).map(factor => factor.label).filter(Boolean),
      note: isPresentationCase
        ? 'No patient-specific risk score is generated for a pre-labelled teaching reference.'
        : (assessmentRisk.explanation || assessmentRisk.notice || 'Assessment concern indicator is not a disease probability or diagnosis.'),
    },
    ...
    scope: isPresentationCase
      ? 'An exact supplied teaching file matched after you enabled Presentation case matching. It is pre-labelled educational content, not AI inference, diagnosis, probability, or disease-risk prediction.'
      : questionnaire
...
function renderPatientResult(data) {
  ...
  const observations = [
    presentation.isPresentationCase ? 'Exact supplied presentation file matched after presentation mode was enabled; no classifier was run.' : '',
